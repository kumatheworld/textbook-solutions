...
\begin{customthm}{II.18.16} (half-done)
  Below is the proof for each item.
  \begin{enumerate}
    \item $\omega$ is a proper initial segment of $\mathrm{nat}^\mathfrak{A}$ because for any $n\in\omega$ and $\nu\in\mathrm{nat}^\mathfrak{A}$ such that $\nu<n$, it holds that $\nu\in\omega$ since $\omega$ is transitive. To see that $\mathrm{nat}^\mathfrak{A}\setminus\omega$ has no least element, consider the formula $\forall x[\mathrm{nat}(x)\wedge x\neq0\rightarrow\exists w[\mathrm{nat}(w)\wedge x=S(w)]]$, which is true in $(M;\in)$ and hence in $(A;E)$ too. That means for any $\nu\in\mathrm{nat}^\mathfrak{A}\setminus\omega$, we have $\mu\in\mathrm{nat}^\mathfrak{A}\setminus\omega$ such that $\mu<S(\mu)=\nu$. Thus $\mathrm{nat}^\mathfrak{A}\setminus\omega$ has no least element.
...
\begin{customthm}{IV.4.14} (half-done)
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \item\underline{$1\in J$}. This is obvious since $1$ has the same decimal expansion as $0\in J$. Coming soon.
