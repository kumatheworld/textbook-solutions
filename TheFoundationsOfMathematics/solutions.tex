...
\begin{customthm}{I.15.12} (half-done)
  Let $\mathfrak{B}\subseteq\mathcal{P}(\mathbb{R})$ be the set of perfect subsets of $\mathbb{R}$. It holds that $|\mathfrak{B}|=\mathfrak{c}$ because $\{{[x,x+1]:x\in\mathbb{R}}\}\subseteq\mathfrak{B}\subseteq\mathfrak{C}$, where $\mathfrak{C}$ is the set of closed subsets of $\mathbb{R}$, which has cardinality $\mathfrak{c}$. We note the fact that every perfect set has cardinality $\mathfrak{c}$ without a proof. The rest of the proof is omitted for now.
\end{customthm}
