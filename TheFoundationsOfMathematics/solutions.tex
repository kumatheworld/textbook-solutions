...
\begin{customthm}{I.12.17} Let $A$ be a set. To prove the 'only if' part, assume $\omega\preceq A$ and let $i:\omega\rightarrow A$ be an injection. Then we can define an injection $f:A\rightarrow A$ with $\mathrm{ran}(f)\neq A$ as follows.
  \[f(x)=
    \begin{cases}
...
To prove the 'if' part, assume $A$ is not Dedekind-finite and let $f:A\rightarrow A$ be an injection with $\mathrm{ran}(f)\neq A$. Then there is $a\in A$ such that $f^n(a)\neq a$ holds for all positive natural number $n$, because otherwise every $a\in A$ would be in the range of $f$ for $a=f(f^{n-1}(a))$. Now fix such $a\in A$ and define $i:\omega\rightarrow A$ by $i(n)=f^{n}(a)$. This is an injection from $\omega$ to $A$ and we have $\omega\preceq A$.
\end{customthm}
