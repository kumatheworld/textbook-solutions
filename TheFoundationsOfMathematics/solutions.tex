...
\begin{customthm}{I.2.1}
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  See the hint in the textbook for Axiom 1 (Extensionality) and Axiom 4 (Pairing). For Axiom 2 (Foundation), $a$ in Example 2 has no element that doesn't intersect itself. That is, the only element $a\in a$ intersects $a$ since they both contain $a$. Similarly, $c$ in Example is a counterexample. Axiom 5 (Union) holds for all the examples since there is a union for every element as shown below.
  \begin{center}
    \begin{tabular}{c|c|c}
      Example & Element & Union \\ \hline
      1 & a & a \\ \hline
      2 & a & a \\ \hline
      \multirow{2}{*}{3} & a & b \\
                         & b & a \\ \hline
...
\begin{customthm}{I.6.3}
  Examples 2, 3 and 4 are the ones. The element $a$ is nonempty in each example.
\end{customthm}

\begin{customthm}{I.6.11}
  Example 7 is the one. Examples 1, 5 and 6 have pairwise unions. Examples 2, 3 and 4 do not have empty sets and hence cannot satisfy Comprehension.
\end{customthm}
...
\begin{customthm}{I.6.17}
  $\langle0,1\rangle=\{\{0\},\{0,1\}\}=\{1,2\}$. $\langle1,0\rangle=\{\{1\},\{1,0\}\}=\{\{1\},2\}$.
\end{customthm}

\begin{customthm}{I.6.20}
  We prove the following.
  \begin{enumerate}
    \item\underline{$\{x,y,z\}=\{x,z,y\}$}. The two sets are equal by Extensionality since it holds that $w\in\{x,y,z\}\leftrightarrow w\in\{x,y\}\cup\{z\}\leftrightarrow w\in\bigcup\{\{x,y\},\{z\}\}\leftrightarrow w\in\{x,y\}\vee w\in\{z\}\leftrightarrow (w=x\vee w=y)\vee w=z\leftrightarrow(w=x\vee w=z)\vee w=y\leftrightarrow w\in\{x,z,y\}$.
    \item\underline{$u\cap(v\cup w)=(u\cap v)\cup(u\cap w)$}. It can be proved similarly using Extensionality, reducing it to the distributive law $p\wedge(q\vee r)=(p\wedge q)\vee(p\wedge r)$ of propositional logic.
  \end{enumerate}
\end{customthm}
...
\begin{customthm}{I.11.31}
  ...
  Now we prove that $\kappa$ is an infinite cardinal iff $\kappa=\aleph_\xi$ for some $\xi$. The 'if' part is obvious from the definition of $\aleph$ and Theorem I.11.17(3). To prove the converse by contradiction, assume there is an infinite cardinal $\kappa$ such that $\forall\xi\in ON\,[\kappa\neq\aleph_\xi]$. Let $\kappa$ be the least cardinal such that $\forall\xi\in ON\,[\kappa\neq\aleph_\xi]$ and let $\xi=\{\eta\in ON:\aleph_\eta<\kappa\}$. Note that $\xi$ is an initial segment of $ON$ by the monotonicity of $\aleph$, which is proven above. Furthermore, $\xi$ forms a set. Otherwise, $\xi=ON$ and $\kappa$ would contain all $\aleph_\eta$, which by Replacement implies $ON$ would be a set. Now we prove $\kappa=\aleph_\xi$. One can easily prove that $\aleph_\xi\leq\kappa$ by splitting $\xi$ into $0$, a successor or a limit ordinal. On the other hand, $\aleph_\xi\not<\kappa$ holds because $\aleph_\xi<\kappa$ would imply $\xi\in\xi$. Thus $\kappa=\aleph_\xi$ holds, which contradicts the definition of $\kappa$. Hence any infinite cardinal is identical to $\aleph_\xi$ for some $\xi$.
\end{customthm}

\begin{customthm}{I.11.33}
  We first prove $\aleph_\gamma\leq\gamma$. Since $\gamma$ is a limit ordinal, $\aleph_\gamma=\sup\{\aleph_\xi:\xi<\gamma\}$ holds and $\aleph_\gamma\leq\gamma$ is equivalent to $\forall\xi<\gamma\,[\aleph_\xi\leq\gamma]$. Let $\xi<\gamma$ and $n\in\omega$ be one such that $\xi<\delta_n$. Then, since $\aleph$ is monotonic (Exercise I.11.31), we have $\aleph_\xi<\aleph_{\delta_n}=\delta_{n+1}<\gamma$. Hence $\aleph_\gamma\leq\gamma$ holds. $\gamma\leq\aleph_\gamma$ is clear from $\forall n\in\omega\,[\delta_n<\aleph_{\delta_n}\leq\aleph_\gamma]$, and we get $\aleph_\gamma=\gamma$.

  Let $\xi$ be an ordinal such that $\aleph_\xi=\xi$. Since $\aleph$ is monotonic, we get $\forall n\in\omega\,[\delta_n\leq\xi]$ by repeatedly applying $\aleph$ to the both side of $0\leq\xi$.  Hence $\gamma\leq\xi$ holds and $\gamma$ turns out to be the least fixed point of $\aleph$.
\end{customthm}
...
\begin{customthm}{I.13.20}
  Let $\lambda$ be an infinite cardinal and $\kappa\leq\lambda$ be a cardinal. We prove the following.
  \begin{enumerate}
    \item\underline{$|[\lambda]^\kappa|$=$\lambda^\kappa$}. (halfway)
    \begin{enumerate}
      \item\underline{$|[\lambda]^\kappa|\leq\lambda^\kappa$}. Consider the surjection $\ran:{}^\kappa\lambda\rightarrow[\lambda]^\kappa\cup[\lambda]^{<\kappa}$ that maps every function to its range. By Lemma I.12.12, there is an injection from $[\lambda]^\kappa\cup[\lambda]^{<\kappa}$ to ${}^\kappa\lambda$. Composing it with the inclusion $[\lambda]^\kappa\subseteq[\lambda]^\kappa\cup[\lambda]^{<\kappa}$, we get an injection $[\lambda]^\kappa\rightarrow{}^\kappa\lambda$.
      \item\underline{$\lambda^\kappa\leq|[\lambda]^\kappa|$}. (halfway) We prove that this holds when $\kappa\leq\cf(\lambda)$. We assume $\kappa\leq\cf(\lambda)$ and consider $F:{}^\kappa\lambda\rightarrow{}^\kappa\lambda$ defined recursively by $F(f)(\xi)=\sup_{\eta<\xi}F(f)(\eta)+1+f(\xi)$ for $\xi<\kappa$. It is well-defined under the assumption $\kappa\leq\cf(\lambda)$. It is also injective, which can be easily shown by induction and left-cancellability of $+$. Moreover, since $F(f)$ is strictly monotonic for each $f$, $\ran\circ F:{}^\kappa\lambda\rightarrow|[\lambda]^\kappa|$ is also injective. Thus, $\lambda^\kappa\leq|[\lambda]^\kappa|$ holds when $\kappa\leq\cf(\lambda)$.
    \end{enumerate}
    \item\underline{$|[\lambda]^{<\kappa}|=\sup\{\lambda^\theta:\theta=|\theta|<\kappa\}$}. It holds that $[\lambda]^{<\kappa}=\bigcup_{\theta=|\theta|<\kappa}[\lambda]^\theta$. Let $\mu=\sup\{\lambda^\theta:\theta=|\theta|<\kappa\}$. 
    \begin{enumerate}
      \item\underline{$|[\lambda]^{<\kappa}|\leq\mu$}. If $\kappa<2$, then both sides equal $\kappa$ and the inequality holds obviously. Now we assume $\kappa\geq2$, in which case $\mu$ is infinite. For each cardinal $\theta\leq\lambda$, it holds that $|[\lambda]^\theta|\leq\mu$ since $|[\lambda]^\theta|=\lambda^\theta$ holds by the above, though not fully proven. On the other hand, $|\{\theta=|\theta|<\kappa\}|\leq\kappa\leq\lambda\leq\mu$ holds. Thus, by Theorem I.12.11, $|[\lambda]^{<\kappa}|\leq\mu$ holds.
      \item\underline{$\mu\leq|[\lambda]^{<\kappa}|$}. This holds since we have $\lambda^\theta=|[\lambda]^\theta|\leq|\bigcup_{\theta=|\theta|<\kappa}[\lambda]^\theta|=|[\lambda]^{<\kappa}|$ for each $\theta=|\theta|<\kappa$.
    \end{enumerate}
  \end{enumerate}
\end{customthm}
...
\begin{customthm}{I.15.10} (halfway)
  We first note that the set of all closed nowhere dense sets, which we denote by $C$ here, has cardinality $\mathfrak{c}=2^{\aleph_0}$. This is because the set of all singletons, which is a subset of $C$, has that cardinality and the set of all closed sets, which is a superset of $C$, has that cardinality too. Let $U$ be the set of all countable unions of $C$. We have $|U|=\mathfrak{c}^{\aleph_0}=\mathfrak{c}$ from the above. Therefore we can list $U$ as $U=\{B_\alpha:\alpha<2^{\omega}\}$. Now we well-order $\mathbb{R}$ and define $X_\alpha\subseteq\mathbb{R}$ for each $\alpha\leq2^{\omega}$ by $X_0=\emptyset$, $X_{\xi+1}=X_\xi\cup\{\min(\mathbb{R}\setminus\{X_\xi\cup\bigcup_{\eta<\xi}B_\eta\})\}$, and $X_{\alpha}=\bigcup_{\xi<\alpha}X_\xi$ for a limit ordinal $\alpha$. \textbf{This definition works if it holds $\forall\xi<2^\omega[\bigcup_{\eta<\xi}B_\eta\neq\mathbb{R}]$, which we currently do not provide a proof for}. Then $X_{2^\omega}$ has cardinality $\mathfrak{c}$, which is uncountable. It has at most $\alpha$ elements from each $B_\alpha$, which is countable if we assume CH. Thus under CH, this $X_{2^\omega}$ is a Luzin set.

  The rest of the exercise is skipped for now.
\end{customthm}

\begin{customthm}{I.15.11} (halfway)
  The existence of a Sierpiński set under CH is skipped for now.

  Let $X\subseteq\mathbb{R}$ be a Sierpiński set. To prove that $X$ cannot be measurable, we show that the measure of $X$ can be neither $0$ nor positive. It is clear that the measure cannot be 0 since $X$ would be countable and uncountable at the same time. To see that $X$ cannot have positive measure, we show that any set of positive measure has a null subset of the same cardinality. The proof is omitted for now.
\end{customthm}

\begin{customthm}{I.15.12} (halfway)
  Let $\mathfrak{B}\subseteq\mathcal{P}(\mathbb{R})$ be the set of perfect subsets of $\mathbb{R}$. It holds that $|\mathfrak{B}|=\mathfrak{c}$ because $\{{[x,x+1]:x\in\mathbb{R}}\}\subseteq\mathfrak{B}\subseteq\mathfrak{C}$, where $\mathfrak{C}$ is the set of closed subsets of $\mathbb{R}$, which has cardinality $\mathfrak{c}$. We well-order $\mathbb{R}$ and list $\mathfrak{B}$ as $\mathfrak{B}=\{B_\alpha:\alpha\in\mathfrak{c}\}$. For each $\alpha\leq\mathfrak{c}$, we define a pair of disjoint subsets $X_\alpha,Y_\alpha\subseteq\mathbb{R}$ by $X_0=Y_0=\emptyset$, $X_{\xi+1}=X_\xi\cup\min(\mathfrak{B}_\alpha\setminus(X_\xi\cup Y_\xi))$, $Y_{\xi+1}=Y_\xi\cup\min(\mathfrak{B}_\alpha\setminus(X_{\xi+1}\cup Y_\xi))$ and $X_{\alpha}=\bigcup_{\xi<\alpha}X_\xi$, $Y_{\alpha}=\bigcup_{\xi<\alpha}Y_\xi$ for a limit ordinal $\alpha$. Note that this definition works as we well-order $\mathbb{R}$ and the set we take the minimum element from is always non-empty. This is because every perfect set has cardinality $\mathfrak{c}$, which we do not provide a proof for. Then, $X_\mathfrak{c}$ is a Bernstein set as both it and its complement $\mathbb{R}\setminus X\supseteq Y_\mathfrak{c}$ meet every set in $\mathfrak{B}$.

  Let $X\subseteq\mathbb{R}$ be a Bernstein set. To prove that $X$ cannot be measurable, we show that any set of positive measure has a perfect subset. Once this is proved, it is easy to see that $X$ is not measurable because at least one of $X$ or $\mathbb{R}\setminus X$ would have a perfect subset, which does not intersect the complement, contradicting the definition of a Bernstein set. The proof is omitted for now.
\end{customthm}
...
\begin{customthm}{II.18.16} (halfway)
  Below is the proof for each item.
  \begin{enumerate}
...
\begin{customthm}{IV.3.31} (halfway)
...
\begin{customthm}{IV.3.33} (halfway)
...
\begin{customthm}{IV.4.13} (halfway)
...
\begin{customthm}{IV.5.2}
  If $\Lambda=\emptyset$, it is trivially a $\Delta_1$ set of sentences of $\mathcal{L}$. Assume $\Lambda\neq\emptyset$ and let $\varphi:\omega\rightarrow HF$ be a computable function such that $\Lambda=\ran(\varphi)$. Let $\psi:\omega\rightarrow HF$ be another computable function defined by $\psi(n)=\wedge\cdots\wedge\varphi(n)\cdots\varphi(n)$ with the Polish notation. Let $\Omega=\ran(\psi)$. It is obvious that $\mathrm{Conseq}(\Lambda)=\mathrm{Conseq}(\Omega)$ as each $\psi(n)$ is equivalent to $\varphi(n)$. To see that $\Omega$ is $\Delta_1$, let $C(\chi)$ be the number of consecutive conjunction operators at the beginning of the given formula $\chi$. Let $C(\chi)=-1$ if $\chi$ is not a formula. $C$ is computable since it can be defined recursively by $C(\wedge\chi_1\chi_2)=1+C(\chi_1)$ and $C(\chi)=0$ if $\chi$ does not start with $\wedge$. Note that $C(\psi(n))=n+C(\varphi(n))\geq n$ holds. Now, $\chi\in\Omega$ is equivalent to $\exists m\leq C(\chi)[\psi(m)=\chi]$, which is $\Delta_1$.
\end{customthm}

\begin{customthm}{IV.5.6}
  Let $\varphi_l=\exists x\forall y[x\leq y]$ and $\varphi_r=\exists x\forall y[y\leq x]$. Let $\psi_0=\varphi_l\wedge(\neg\varphi_r)$, $\psi_1=(\neg\varphi_l)\wedge(\neg\varphi_r)$, $\psi_2=(\neg\varphi_l)\wedge\varphi_r$, and $\psi_3=\varphi_l\wedge\varphi_r$. Since $\psi_0\vee\psi_1\vee\psi_2\vee\psi_3$ is always true, $\Lambda\vdash\chi$ holds iff $\Lambda\cup\{\psi_i\}\vdash\chi$ for each $i\in4$. By Exercise II.13.10, each $\Lambda\cup\{\psi_i\}$ is complete, so each $\Lambda\cup\{\psi_i\}\vdash\chi$ has a $\Delta_1$ graph by Lemma IV.5.5, which shows the decidability of the conjunction $\Lambda\vdash\chi$.
\end{customthm}
