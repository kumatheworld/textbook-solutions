...
\begin{customthm}{IV.4.14} (half-done)
  ...
  \begin{enumerate}
    ...
    \end{enumerate}
    \item\underline{$x\in J\rightarrow\sin(x),e^x\in J$}. Let $x\in J$. We prove that $\sin(x)\in J$. By coupling adjacent terms of its Taylor series, we have $\sin(x)=\sum_{k=0}^\infty(-1)^kx^{2k+1}/(2k+1)!=\sum_{l=0}^\infty s(l,x)$, where $s(l,x)=(1-x^2/(2l+1)(2l+3))x^{2l+1}/(2l+1)!$. For any $n\in\omega$, the partial sum $\sum_{l=0}^n s(l,x)$ is computable as it is constructed from computable numbers $l$ and $x$ using the four basic arithmetic operations, which we know from above make the results of the operations also computable.
