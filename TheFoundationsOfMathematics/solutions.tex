  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \item\underline{$y=d\cdot10^k$ for some $d\in10$ and $k\in\mathbb{Z}$}. It holds that $w\in J\leftrightarrow10w\in J$ since one is just the shifted one of the other. Apply this repeatedly to the previous case.
        ...
        \item\underline{$y\in J$}. We may assume that $|x|,|y|\leq1$ since $w\in J\rightarrow w/10\in J$ holds as shown above. Let $v=T(i,u)$ say that $v$ is the real number that truncates $u$ at the $i$-th digit, followed by all $0$'s after that. It is obvious that $T$ defines a computable function. Now, it holds that $d$ is the $n$-th digit of $x\cdot y$ iff $d<10\wedge[d\equiv(x\cdot T(n,y))(n)\vee d\equiv(x\cdot T(n,y))(n)+1]\wedge[d\equiv(x\cdot T(n,y))(n)+1\rightarrow\exists m[n<m\wedge d\equiv(x\cdot T(m,y))(n)+1]]$, where $\equiv$ is again the equivalence modulo $10$. Intuitively, this says that $x\cdot T(n,y)$ almost determines the $n$-th digit of $x\cdot y$ since $|x|,|y|\leq1$, and the digit increases by 1 modulo 10 if and only if there are some more digits in $y$ that contribute to the increment. Since the formula is $\Sigma_1$, the relation is $\Delta_1$ as any decimal expansion is a function, meaning that $x+y\in J$.
      \end{enumerate}
    \end{enumerate}
